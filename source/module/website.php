...
function get_websites($cate_id = 0, $top_num = 10, $is_pay = false, $is_best = false, $field = 'ctime', $order = 'desc') {
	...
	while ($row = $DB->fetch_array($query)) {
		...
		$row['web_pic'] = get_webthumb($row['web_pic'],$web['web_url']);
...
/** website list */
function get_websites_status($cate_id = 0, $top_num = 10, $status = 3) {
	global $DB;
	
	$sql = "SELECT w.web_name, w.web_url, w.web_ctime FROM ".$DB->table('websites')." w WHERE w.web_status=".$status." ORDER BY web_ctime desc $order LIMIT $top_num";
	$query = $DB->query($sql);
	$results = array();
	while ($row = $DB->fetch_array($query)) {
		$row['web_ctime'] = date('Y-m-d', $row['web_ctime']);
...
/** sitemap txt格式 */
function get_website_sitemap_type($pageNum=0,$pagesize=0,$type=0) {
	global $DB;
	$where = "web_status=3";
	$con = "";
	if($pagesize>0){
		$con =" LIMIT ".$pageNum.$pagesize;
	}
	$sql = "SELECT web_id,web_ctime FROM ".$DB->table('websites');
	$sql .= " WHERE $where ORDER BY web_id DESC $con";
	$query = $DB->query($sql);
	$sm_txt="";
	while ($row = $DB->fetch_array($query)) {
		if($type==1){
			$sm_txt .=pro_sitemap_xml(str_replace('&', '&amp;', get_website_url($row['web_id'], true)),$row['web_ctime']);
		}else{
			$sm_txt = $sm_txt.str_replace('&', '&amp;', get_website_url($row['web_id'], true))."\n";
		}
	}
	unset($row);
	$DB->free_result($query);
