...
function get_webthumb($web_pic,$web_url='') {
	...
	if (!empty($web_pic)) {
		$strurl = $options['site_root'].$options['upload_dir'].'/website/'.$web_pic;
	}else{
		if($options['is_enabled_connect']=='yes'){
			$strurl = "http://images.shrinktheweb.com/xino.php?stwembed=1&stwxmax=120&stwymax=90&stwdelay=5&stwaccesskeyid=".$options['qq_appid']."&stwurl=http://".$web_url;
		}
	} 
	return $strurl;
}
