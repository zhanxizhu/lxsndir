...
/** weblink sitemap */
function get_weblink_sitemap($type=0) {
	global $DB, $deal_types;
	$sql = "SELECT l.link_id,l.link_name FROM ".$DB->table('weblinks')." l order by link_id asc";
	$results = $DB->fetch_all($sql);
	$sm_txt="";
	foreach ($results as $row) {
		if($type ==1){
			$sm_txt .=pro_sitemap_xml($url.get_weblink_url($row['cate_id'],true),time(),"weekly",0.6);
		}else{
			$sm_txt = $sm_txt.$url.get_weblink_url($row['cate_id'],true)."\n";
		}
	}
	unset($results);
	return $sm_txt;
