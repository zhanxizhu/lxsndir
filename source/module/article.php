...
/** sitemap */
function get_article_sitemap_type($type = 0) {
	global $DB;
	
	$where = "art_status=3";
	$sql = "SELECT art_id, art_ctime FROM ".$DB->table('articles');
	$sql .= " WHERE $where ORDER BY art_id DESC";
	$query = $DB->query($sql);
	$sm_txt="";
	while ($row = $DB->fetch_array($query)) {
		if($type ==1){
			$sm_txt .=pro_sitemap_xml(str_replace('&', '&amp;', get_article_url($row['art_id'], true)),$row['art_ctime']);
		}else{
			$sm_txt = $sm_txt.str_replace('&', '&amp;', get_article_url($row['art_id'], true))."\n";
		}
	}
	unset($row);
	$DB->free_result($query);
