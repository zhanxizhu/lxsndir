...
/** all category  sitemap txt格式 */
function get_category_sitemap($url,$type=0) {
	global $DB;
	$results = load_cache('categories') ? load_cache('categories') : $DB->fetch_all("SELECT cate_id, cate_mod, cate_dir FROM ".$DB->table('categories')." ORDER BY cate_id ASC");
	$sm_txt="";
	foreach ($results as $row) {
		if($type ==1){
			$sm_txt .=pro_sitemap_xml($url.get_category_url($row['cate_mod'], $row['cate_id']),time(),"daily",0.8);
		}else{
			$sm_txt = $sm_txt.$url.get_category_url($row['cate_mod'], $row['cate_id'])."\n";
		}
