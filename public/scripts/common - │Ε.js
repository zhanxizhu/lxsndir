...
function rewrite_search(){
	...
	if (query == "") {
		...
		if (rewrite == 1) {
			window.location.href = sitepath + "search-name-" + encodeURI(query) + ".html";
		} else if (rewrite == 2) {
			window.location.href = sitepath + "search/name/" + encodeURI(query) + ".html";
		} else if (rewrite == 3) {
			window.location.href = sitepath + "search/name/" + encodeURI(query);
		} else {
			this.form.submit();
		}
	}
...
function getscreenshot() {
	...
	$(document).ready(function(){$("#img_btn").val('正在获取，请稍候...'); $.ajax({type: "GET", url: sitepath + '?mod=ajaxget&type=screen', data: 'url=' + url, datatype: "script", cache: false, success: function(data){$("body").append(data); $("#img_btn").val('重新获取');}});});	
}
...
function rate(j){
	...
	if ($.cookie('rate_aid_'+artid) == 1) {
		...
		setTimeout(function(){$("#RateMsg").hide('slow');}, 2000);
	} else {
		$.getJSON(sitepath + "?mod=getdata&type=score&wid="+wid+"&j="+j+ "&r="+Math.random(),
		function(data) {
